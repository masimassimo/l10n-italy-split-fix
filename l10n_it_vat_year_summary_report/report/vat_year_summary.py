...
class VatYearSummaryReport(report_sxw.rml_parse):
    _name = 'report.vat.year.summary'

    def __init__(self, cr, uid, name, context=None):
        if context is None:
            context = {}
        super(VatYearSummaryReport, self).__init__(cr, uid, name, context=context)
    ...
    def _get_tax_codes_amounts(self, type='credit',
        ...
        context['fiscalyear_id'] = \
            self.localcontext['data']['form']['fiscalyear_id']
    ...
    def _get_account_vat_amounts(self, type='credit', context={}):
        tax_codes = self._get_tax_codes_amounts(type)
        total_amounts = 0
        data = {
            'type' : type,
            'amount' : 0,
            }
        for tax_account in tax_codes:
            tax = tax_codes[tax_account]
            total_amounts += tax['vat_deductible']
    ...
    def _get_lines_generic_amounts(self, context={}):
        ...
        res = []
        fiscalyear_id = self.localcontext['data']['form']['fiscalyear_id']
    ...
    def get_year(self, context={}):
        return self.pool['account.fiscalyear'].browse(
            self.cr, self.uid,
            self.localcontext['data']['form']['fiscalyear_id']).name
            
    def get_amount_interest(self, context={}):
        return self.localcontext['data']['form']['amount_interest']
    def get_amount_generic(self, context={}):
        return self.localcontext['data']['form']['amount_generic']
    def get_amount_paid(self, context={}):
        amount = self.localcontext['data']['form']['amount_paid'] 
        return amount
    
    def get_page_year(self, context={}):
        page_year = ''
        if 'print_page_year' in self.localcontext['data']\
                and self.localcontext['data']['form']['print_page_year']:
            page_year = self.localcontext['data']['form']['print_page_year']
        return page_year
    
    def get_page_number_from(self, context={}):
        page_number_from = 0
        if 'print_page_number_from' in self.localcontext['data']['form']\
                and self.localcontext['data']['form']['print_page_number_from']:
            page_number_from = \
                self.localcontext['data']['form']['print_page_number_from']
        return page_number_from


class ReportVatYearSummary(orm.AbstractModel):
    _name = ('report.l10n_it_vat_year_summary_report.report_vatyearsummary')
    _inherit = 'report.abstract_report'
    _template = 'l10n_it_vat_year_summary_report.report_vatyearsummary'
    _wrapped_report_class = VatYearSummaryReport